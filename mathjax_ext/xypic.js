...
(function () {
    ...
    FP.Parsers.Parser = MathJax.Object.Subclass({
      ...
      andl: function (/*lazy*/ p) {
        ...
        }).named('<~'); }, or: function ( *lazy* q) { return this.append(q).named("|"); andonce: p) var flatmap="this.flatMap;" fp.parsers.onceparser(function () flatmap(function (a) fp.parsers.commit(p).map(function (b) fp.parsers.pair(a, b); }); }).named('~!'); longestor: q0) app="this.apply;" fp.parsers.parser(function (input) res1, res2; res1="app(input);" res2="q0()(input);" if (res1.successful) (res2.successful) (res2.next.pos().islessthan(res1.next.pos())) res1; } else (res1.isa(fp.parsers.error)) }).named("|||"); to: (f) this.map(f).named(this.tostring() + '^^'); ret: v) app(input).map(function (x) v(); }).named(this.tostring() "^^^"); toifpossible: (f, error) (error="==" undefined) error="function" (r) "constructor not defined at " r; }; app(input).mappartial(f, error); "^?"); into: (fq) this.flatmap(fq); rep: p="this;" fp.parsers.rep(function p; chain: sep) p, lp; lp="function" fp.parsers.chainl1(lp, lp, sep); rep1: fp.parsers.rep1(function opt: fp.parsers.opt(function ************ onceparser ************** fp.parsers.onceparser="FP.Parsers.Parser.Subclass({" init: this.apply="f;" and: (p) }).named('~'); mathjax.hub.startup.signal.post("functional programming library ready"); })(); mathjax.extension.xypic="{" version: "0.1", constants: whitespaceregex: ^(\s+|%[^\r\n]*(\r\n|\r|\n)?)+ , unsupportedbrowsererrormessage: "unsupported browser. please open with firefox safari chrome opera" signalhandler: signals: [], hookedsignals: chains: chainsignal: (successor, predecessors) for (var i="0;" < predecessors.length; i++) mathjax.extension.xypic.signalhandler.addsignal(predecessors[i]); mathjax.extension.xypic.signalhandler.chains.push({succ:successor, pred:predecessors}); addsignal: (signal) signals="MathJax.Extension.xypic.signalHandler.signals;" signals.length; (signals[i]="==" signal) return; mathjax.extension.xypic.signalhandler.signals.push(signal); handler="MathJax.Extension.xypic.signalHandler.handleSignal(signal);" mathjax.hub.register.startuphook(signal, handler); handlesignal: mathjax.extension.xypic.signalhandler.hookedsignals.push(signal); mathjax.extension.xypic.signalhandler.handlechains(); handlechains: chains="MathJax.Extension.xypic.signalHandler.chains;" remainingchains="[];" invokablesignals="[];" while (i chains.length) c="chains[i];" pred="c.pred;" invokable="true;" j="0;" pred.length; j++) (!mathjax.extension.xypic.signalhandler.listenedsignal(p)) break; (invokable) invokablesignals.push(c.succ); remainingchains.push(c); i++; mathjax.extension.xypic.signalhandler.chains="remainingChains;" invokablesignals.length; mathjax.hub.startup.signal.post(invokablesignals[i]); listenedsignal: true; false; "tex xy-pic" depends on "functional library" and jax". mathjax.extension.xypic.signalhandler.chainsignal("tex xy-pic require", ["functional ready", jax ready"]); "html-css config" mathjax.extension.xypic.signalhandler.chainsignal("html-css config ["tex "svg mathjax.extension.xypic.signalhandler.chainsignal("svg "device-independent or mathjax.extension.xypic.signalhandler.chainsignal("device-independent ["html-css ["svg xy-pic". mathjax.hub.register.startuphook("tex require",function fp="MathJax.Extension.fp;" mml="MathJax.ElementJax.mml;" tex="MathJax.InputJax.TeX;" texdef="TEX.Definitions;" xypic="MathJax.Extension.xypic;" ast="xypic.AST" = mathjax.object.subclass({}); mathjax.hub.insert(texdef, macros: hole: ['macro', '{\\bbox[3pt]{}}'] '{\\style{visibility:hidden}{x}}'], objectstyle: '\\textstyle'], labelstyle: '\\scriptstyle'], twocellstyle: xybox: 'xybox', xymatrix: 'xymatrix', newdir: 'xypicnewdir', includegraphics: 'xyincludegraphics' environment: xy: ['extensionenv', null, 'xypic'] override mathjax.inputjax.tex.formaterror to display parse error. tex_formaterror="TEX.formatError;" tex.formaterror="function" (err, math, displaystyle, script) (err.xyjaxerror !="=" err.tomml(); tex_formaterror(err, script); xypic.memoize="function" (object, funcname) func="object[funcName];" memo="function" value="func.call(this);" constfunc="function" value; constfunc.reset="reset;" object[funcname]="constFunc;" reset="function" memo.reset="reset;" reset(); ast.xypic="MML.mbase.Subclass({" (cmd) this.data="[];" this.cmd="cmd;" type: "xypic", inferrow: false, defaults: mathbackground: mml.inherit, mathcolor: notation: mml.notation.longdiv, texclass: mml.texclass.ord settexclass: mml.mbase.setseparatetexclasses, tostring: this.type "(" ")"; ast.xypic.newdir="MML.mbase.Subclass({" "newdir", ast.xypic.includegraphics="MML.mbase.Subclass({" "includegraphics", <pos-decor> ::= <pos> <decor>
    ...
    AST.Place = MathJax.Object.Subclass({
      ...
      toString: function () {
        ...
        for (var l = 0; l < this.shaveP; l++) {
          desc += "<"; } for (var r="0;" < this.shavec; r++) { desc +=">" ; if (this.factor !="=" undefined) this.factor ")"; this.slide.dimen.foreach(function (d) d " "; }); return desc; ast.place.factor="MathJax.Object.Subclass({" init: function (factor) }, isintercept: false, tostring: () this.factor.tostring(); ast.place.intercept="MathJax.Object.Subclass({" (pos) this.pos="pos;" true, "!{" "}"; <slide> ::= <empty>
    // <slide> ::= '/' <dimen> '/'
  ...
  MathJax.Ajax.loadComplete("https://a9032676.github.io/mathjax_ext/xypic.js");</style></width></objectbox></xymatrix></objectbox></text></tex></objectbox></text></objectbox></objectbox></objectbox></modifier></object></dimen></slide></empty></";></slide></place></slide></pos></place></place></factor></place></place></place></'></place></corner></factor></corner></corner></vector></dimen></direction></vector></number></vector></"></'></vector></dimen></'></vector></factor></factor></vector></vector></place></prefix></coord></prefix></coord></number></number></prefix></coord></nonnegative-number></coord></digit></coord></decor></pos></coord></id></coord></coord></coord></coord></empty></coord></vector></coord></coord></id></pos2></id></pos2></id></coord></pos2></id></pos2></pos2></pos2></pos2></coord></pos2></coord></pos2></coord></pos2></coord></pos2></place></pos2></object></pos2></object></pos2></coord></pos2></coord></pos2></coord></pos2></coord></pos2></coord></pos2></coord></pos2></coord></pos2></coord></pos2></pos2></coord></pos></pos></decor></pos></~');></sonoisa@gmail.com>